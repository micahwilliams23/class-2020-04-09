...
predict(model_1, newdata = data.frame(faculty = 50, sat = 1200))
```
...
model_1 %>%
  augment(x) %>% 
  filter(.resid %in% c(min(.resid),max(.resid)))
```
...
Without interaction:
On average, graduates of public schools made 14.9 thousand dollars more per year on average. Every additional dollar increase in price corresponded to a 1.39 dollar increase in earnings.

With interaction:
On average, graduates of public schools made 11.1 thousand dollars more per year on average. Every additional dollar increase in price corresponded to a 1.35 dollar increase in earnings,
plus another 0.97 dollars if the student attended a public school.
...
rep_sample_n(x, size = nrow(x), reps = 100, replace = T) %>% 
  group_by(replicate) %>%
  nest() %>%
  mutate(mod = map(data, ~ tidy(lm(earnings ~ public * price, data = .)))) %>%
  unnest(mod) %>%
  group_by(term) %>%
  # pull(estimate) %>%
  summarize(conf.int = list(quantile(estimate, c(0.025,0.975)))) %>%
  mutate(lower = map_dbl(conf.int, ~ .[1],),
         upper = map_dbl(conf.int, ~ .[2]))
